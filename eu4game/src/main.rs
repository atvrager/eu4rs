...
impl App {
    ...
    async fn new(window: winit::window::Window) -> Self {
        ...
        // Load terrain assets for RealTerrain map mode
    ...
    fn render(&mut self) -> Result<(), wgpu::SurfaceError> {
        ...
        self.renderer.update_map_mode(
            &self.queue,
            map_mode_value,
            self.renderer.map_size, // Use map dimensions, not window size
            current_zoom,
        ...
        // Render pass (always uses depth attachment - required for all pipelines)
        {
            ...
            let country_resources = self.player_tag.as_ref().and_then(|tag| {
                self.world_state
                    .as_ref()
                    .and_then(|ws| world_loader::extract_country_resources(ws, tag))
            });
