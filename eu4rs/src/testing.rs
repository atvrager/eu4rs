...
pub fn assert_snapshot(actual: &RgbaImage, name: &str) {
    ...
    assert_snapshot_at(actual, name, &golden_dir);
}

fn assert_snapshot_at(actual: &RgbaImage, name: &str, golden_dir: &Path) {
...
mod tests {
    ...
    fn test_assert_snapshot_match() {
        ...
        assert_snapshot_at(&img, "test_snap", golden_dir);
        ...
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            assert_snapshot_at(&img_bad, "test_snap", golden_dir);
        }));
